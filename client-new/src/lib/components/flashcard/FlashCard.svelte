          ...
          <div class="text-center space-y-3 w-full max-w-md">
            ...
            {#if hintLevel > 0}
              <div class="p-3 bg-yellow-50/50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                <p class="text-xs text-teal-600 dark:text-teal-400 mb-1">💡 Hint {hintLevel}/3:</p>
                <p class="text-2xl font-mono tracking-wider text-teal-700 dark:text-teal-300">{hintText}</p>
              </div>
            {/if}
              ...
              {#each quizOptions as option, index}
                <button
                  ...
                  class={cn(
                    ...
                    'border-2',
                    showResult && option === word.definition
  ...
  {:else}
    ...
    <div class="flashcard-face absolute inset-0 backface-hidden rotate-y-180">
      ...
      >
        {#if word}
          <div class="text-center space-y-2">
            <h2 class="text-4xl font-bold text-emerald-900 dark:text-emerald-50">
              {word.word}
            </h2>
