...
<header class="sticky top-0 z-50 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm">
  <div class="container mx-auto px-4 py-4">
    <div class="flex items-center justify-between">
      <!-- Logo / Brand -->
      <a href="/" use:link class="flex items-center gap-3 hover:opacity-80 transition-opacity">
      ...
      <div class="flex items-center gap-4">
        ...
        >
          {#if $theme === 'light'}
            <Moon class="w-5 h-5 text-gray-700 dark:text-slate-300" />
          {:else}
            <Sun class="w-5 h-5 text-gray-700 dark:text-slate-300" />
          {/if}
        </button>
